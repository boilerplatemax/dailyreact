    ...
    <>
      ...
      >
        <div className="p-6">
          <div className="flex flex-row justify-between">
            ...
            <div>
              <Link href={`/store/${props.userId}`} passHref>
                <a className={s.link} rel="noopener noreferrer" target="_blank">
                  Preview
                </a>
              </Link>
            </div>
          ...
          <p className="mt-8">
            <span className="text-base font-medium text-zinc-100"></span>
          </p>
        </div>
      </div>
    </>
...
const StoreList = ({ user }: { user: User }) => {
  ...
  return (
    <div className="mt-12 sm:mt-16 sm:grid sm:grid-cols-2 sm:gap-6 lg:max-w-4xl lg:mx-auto xl:max-w-none xl:mx-0 xl:grid-cols-2">
      {userDetails?.storedata?.stores?.map((store) => {
        return <Store store={store} userId={userId} />;
      })}
    </div>
  );
};
